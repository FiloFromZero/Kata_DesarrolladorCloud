            ...
            <table class="min-w-full">
              <thead class="bg-slate-50 text-slate-600 text-xs">
                <tr>
                  ...
                  <th class="px-3 py-2 text-left font-medium">Comentarios</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 text-sm">
                @for (h of history; track h.timestamp) {
                  <tr>
                    <td class="px-3 py-2 text-slate-600">{{ h.timestamp }}</td>
                    <td class="px-3 py-2 text-slate-700">{{ h.actor }}</td>
                    <td class="px-3 py-2">
                      <span class="px-2 py-1 text-xs rounded-md ring-1" [ngClass]="stateStyles[h.status]">{{ statusLabel(h.status) }}</span>
                    </td>
                    <td class="px-3 py-2 text-slate-700">{{ h.comments || '-' }}</td>
                  </tr>
                }
              </tbody>
            </table>
