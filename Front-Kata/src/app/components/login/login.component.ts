        ...
        <form class="p-6 space-y-4" [formGroup]="form" (ngSubmit)="onSubmit()">
          <div>
            ...
            <input formControlName="username" type="text" autocomplete="username"
                   [disabled]="loading"
                   class="w-full rounded-lg border border-gray-200 bg-slate-50 text-slate-800 p-3 outline-none focus:ring-2 focus:ring-[#0b4dbb] focus:border-[#0b4dbb]/40 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <button type="submit" [disabled]="loading || form.invalid"
                  class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-[#0b4dbb] text-white hover:bg-[#093d99] disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0b4dbb] focus-visible:ring-offset-2 transition-all">
            @if (loading) {
